...
//  Descricao:
//    Este TAD eh o responsavel por todas as criacoes e interacoes entre matrizes ou entre ponto e matriz em 2d

#ifndef __MATRIZES_2D_H__
#define __MATRIZES_2D_H__
...
void Ponto2d::AtualizaCoordenadas()
{
  coordenadas[0] = x;
  coordenadas[1] = y;
  coordenadas[2] = 1;
...
void Ponto2d::SetCoordenadas(float x, float y)
{
  this->x = x;
  this->y = y;
  AtualizaCoordenadas();
...
void Ponto2d::MultiplicadoPorMatriz(float mat[3][3])
{
  float res[3];
  for (int i = 0; i < 3; i++)
  {
...
public:
  ...
  Matriz2d();         // construtor, sempre inicia a matriz como matriz identidade
...
void Matriz2d::CopiaMatriz(float mat[3][3])
{
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
...
void Matriz2d::MultiplicaMatriz(float mat[3][3])
{
  float res[3][3];
  for (int i = 0; i < 3; i++)
  {
...
void Matriz2d::MultiplicadoPorMatriz(float mat[3][3])
{
  float res[3][3];
  for (int i = 0; i < 3; i++)
  {
...
void Matriz2d::MatrizRotacao(float rad)
{
  float rot[3][3];
...
void Matriz2d::MatrizTranslacao(float x, float y)
{
  float trans[3][3];
...
void Matriz2d::MatrizEscala(float multiX, float multiY)
{
  float escala[3][3];
...
void Matriz2d::MatrizRotaEmEixo(float rad, float centroX, float centroY)
{
